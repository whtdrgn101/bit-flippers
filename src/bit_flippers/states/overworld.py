...
from bit_flippers.sprites import create_placeholder_enemy, load_player
from bit_flippers.npc import make_npc
from bit_flippers.items import Inventory, Equipment
...
from bit_flippers.encounter import EncounterManager
from bit_flippers.interaction import resolve_npc_interaction
...
class OverworldState:
    ...
    def _restore_from_save(self, save_data):
        ...
        self.encounters.steps_since_encounter = save_data.get("steps_since_encounter", 0)
    ...
    def _try_interact(self):
        ...
        # Check tile events (chests, signs, switches, etc.)
    ...
    def _interact_with_npc(self, npc):
        """Resolve an NPC interaction via InteractionHandler and push states."""
        from bit_flippers.states.dialogue import DialogueState
        from bit_flippers.quests import QUEST_REGISTRY

        result = resolve_npc_interaction(npc, self.player_quests, self.inventory)
        on_close = None

        if result.quest_action == "accept":
            qid = result.quest_id
        ...
        elif result.quest_action == "claim_rewards":
            qid = result.quest_id
            def on_close(_ow=self, _qid=qid):
        ...
        elif result.open_default_shop:
            def on_close(_ow=self):
                from bit_flippers.states.shop import ShopState
                _ow.game.push_state(ShopState(_ow.game, _ow))

        elif result.shop_stock is not None:
            stock = result.shop_stock
            def on_close(_ow=self, _stock=stock):
                from bit_flippers.states.shop import ShopState
                _ow.game.push_state(ShopState(_ow.game, _ow, stock_list=_stock))

        self.game.push_state(
            DialogueState(self.game, result.npc_name, result.dialogue_lines, on_close=on_close)
        )
    ...
    def _start_random_combat(self, enemy_data):
        from bit_flippers.states.transition import CombatTransition
    ...
    def _handle_tile_event(self, event):
        """Handle a triggered tile event via EventManager.execute()."""
        action = self.event_manager.execute(
            event,
            quest_states=self.player_quests.states if hasattr(self.player_quests, 'states') else None,
        ...
        if action.sfx:
            self.game.audio.play_sfx(action.sfx)

        if action.mark_triggered:
            self.event_manager.mark_triggered(event.x, event.y)

        if action.action_type == "give_item":
            self.inventory.add(action.item_name)
            self.pickup_message = action.message
            self.pickup_message_timer = PICKUP_MESSAGE_DURATION

        elif action.action_type == "dialogue":
            from bit_flippers.states.dialogue import DialogueState
            self.game.push_state(DialogueState(self.game, "Sign", action.dialogue_lines))

        elif action.action_type == "damage":
            self.stats.current_hp = max(1, self.stats.current_hp - action.damage)
            self.pickup_message = action.message
            self.pickup_message_timer = PICKUP_MESSAGE_DURATION
        ...
        elif action.action_type == "toggle_walkable":
            tx, ty = action.target_x, action.target_y
            if 0 <= tx < self.tiled_renderer.width_tiles and 0 <= ty < self.tiled_renderer.height_tiles:
                current = self.tiled_renderer._walkable[ty][tx]
                self.tiled_renderer._walkable[ty][tx] = not current
            self.pickup_message = action.message
            self.pickup_message_timer = PICKUP_MESSAGE_DURATION

        elif action.action_type == "teleport":
            self.player_x = action.target_x
            self.player_y = action.target_y
            self.player_visual_x = float(action.target_x * TILE_SIZE)
            self.player_visual_y = float(action.target_y * TILE_SIZE)

        elif action.action_type == "start_combat":
            from bit_flippers.combat import ENEMY_TYPES
            if action.enemy_type in ENEMY_TYPES:
                self._start_random_combat(ENEMY_TYPES[action.enemy_type])

        elif action.action_type == "heal":
            if action.heal_hp:
                self.stats.current_hp = min(self.stats.max_hp, self.stats.current_hp + action.heal_hp)
            if action.heal_sp:
                self.stats.current_sp = min(self.stats.max_sp, self.stats.current_sp + action.heal_sp)
            self.pickup_message = action.message
            self.pickup_message_timer = PICKUP_MESSAGE_DURATION

        elif action.action_type == "give_scrap":
            if action.scrap_amount:
                self.stats.money += action.scrap_amount
            if action.item_name:
                self.inventory.add(action.item_name)
            self.pickup_message = action.message
            self.pickup_message_timer = PICKUP_MESSAGE_DURATION
    ...
    def _try_move(self, key):
        ...
        if self.tiled_renderer.is_walkable(new_x, new_y) and not self._npc_at(new_x, new_y):
            ...
            # Step-on events (traps, damage zones, teleports, etc.)
            step_event = self.event_manager.on_step(new_x, new_y)
            if step_event is not None:
                self._handle_tile_event(step_event)

            # Random encounter check via EncounterManager
            enemy_data = self.encounters.on_step()
            if enemy_data is not None:
                self._start_random_combat(enemy_data)
